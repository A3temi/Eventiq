...
export function OverviewTab({ event, onOpenVendor, onUpdateEvent }: Props) {
  const fileRef = useRef<HTMLInputElement>(null);

  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !onUpdateEvent) return;
    const reader = new FileReader();
    reader.onload = () => onUpdateEvent({ coverImage: String(reader.result) });
    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="relative rounded-2xl overflow-hidden border border-border bg-muted/40 h-40 sm:h-56 group">
        ...
        <button
          ...
          className="absolute top-3 right-3 inline-flex items-center gap-1.5 rounded-lg bg-background/90 backdrop-blur px-3 py-1.5 text-xs font-medium border border-border shadow-sm hover:bg-background"
        >
          <ImagePlus className="h-3.5 w-3.5" />
          {event.coverImage ? "Change photo" : "Upload photo"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <KeyCard
          ...
          label="Budget"
          value={event.budget ? `£${event.budget.spent.toLocaleString()}` : "—"}
          sub={event.budget ? `of £${event.budget.total.toLocaleString()}` : undefined}
        />
        <KeyCard
