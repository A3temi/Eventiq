...
import { Plus, Pin, Users, Wallet, Filter, X, ChevronDown, Camera } from "lucide-react";
        ...
        {filtered.map((evt) => {
          ...
          return (
            <button
              ...
              className="text-left rounded-2xl border border-border bg-card overflow-hidden hover:border-primary/40 hover:shadow-sm transition group"
            >
              <div className="h-28 w-full bg-muted/40 overflow-hidden">
                {evt.coverImage ? (
              ...
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {format(parseISO(evt.date), "EEE, MMM d, yyyy")}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <Stat icon={<Wallet className="h-3.5 w-3.5" />} label="Budget" value={evt.budget ? `£${(evt.budget.spent / 1000).toFixed(1)}k` : "—"} />
